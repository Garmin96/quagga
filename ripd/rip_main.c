...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    case 0:
      break;
    case 'd':
      daemon_mode = 1;  /* 以守护进程方式运行 */
      break;
    case 'f':
      config_file = optarg; /* 设置配置文件路径 */
      break;
    case 'A':
      vty_addr = optarg;  /* 设置vty的绑定地址 */
      break;
    case 'i':
      pid_file = optarg;  /* 设置进程标识文件路径 */
      break;
    case 'z':
      zclient_serv_path_set(optarg);  /* 设置zebra socket路径 */
      break;
    case 'P': /* 设置vty的端口号 */
      ...
      break;
    case 'r':
      retain_mode = 1;  /* 保留添加的路由 */
      break;
    case 'C':
      dryrun = 1;     /* 检查配置文件的有效性并退出 */
      break;
    case 'u':
      ripd_privs.user = optarg; /* 设置运行用户 */
      break;
    case 'g':
      ripd_privs.group = optarg;  /* 设置运行用户组 */
      break;
    case 'v':
      print_version(progname);  /* 打印程序版本 */
      exit(0);
      break;
    case 'h':
      usage(progname, 0); /* 显示帮助信息 */
      break;
    default:
      usage(progname, 1); /* 显示帮助信息并退出 */
      break;
    }
  }
